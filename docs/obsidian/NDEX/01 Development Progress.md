...
현재 릴리스는 0.9.3 Beta. 0.9.2의 Phase 0~6, 동시 기록 보호, Launcher 개선에
PR #18의 파일 복사 안전성과 백업 상태 정확성 수정을 포함한다.
다음 패치는 [[Roadmap]]의 7B 작업 취소·백그라운드 처리·안전한 종료를 우선한다.
7B 이후는 계획이며 완료로 간주하지 않는다.
